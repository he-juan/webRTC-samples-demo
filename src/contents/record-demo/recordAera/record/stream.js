 ...
 * @returns {Promise<*>}
 */
Record.prototype.getCaptureStreams = async function (canvas){
    console.info("Prepare to obtain other streams through CaptureStream!")
    let stream
    if(!canvas){
    ...
    try{
        if(canvas.captureStream){
            ...
            console.info('Current browser does not support captureStream!!')
            console.error({codeType: 'Current browser does not support captureStream!! can not send whiteboard by slides'})
        }
        console.info("get captureStream: ", stream);
    }catch(e){
        console.error("canvas captureStream get " + e.name)
        console.info("can not canvas stream and can share screen")
    }
    return stream
}
