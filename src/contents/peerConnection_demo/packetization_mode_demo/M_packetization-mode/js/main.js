...
async function start(set){
    let localSlidesStream = await navigator.mediaDevices.getDisplayMedia(constraints)
    let remoteSlidesStream = await navigator.mediaDevices.getDisplayMedia(constraints)
    ...
    await localSlidesStream.getTracks().forEach(track => localPeerConnection.addTrack(track, localSlidesStream))  // Slides
    ...
    await remoteSlidesStream.getTracks().forEach(track => remotePeerConnection.addTrack(track, remoteSlidesStream))  // Slides

	try {
		let offerSdp = await localPeerConnection.createOffer(offerConstraints)
        // offerSdp = getLocalSDP(offerSdp.sdp)
		...
		if(stream){
			if(local){
				if(!localVideo.srcObject){
                    ...
                    let remote = document.getElementById('local_video')
				...
				if(!remoteVideo.srcObject){
                    ...
                    remote_video.onloadedmetadata=function(){
                        console.warn("456456456455")
                        remote_video.play()
                    }
				}
...
async function localOnIceCandidate(event){
	...
	if (iceState === 'completed' || iceState === 'complete' || (event && !event.candidate)) {
		...
		// let offerSdp = localPeerConnection.localDescription.sdp
...
async function remoteOnIceCandidate(event){
	...
	if (iceState === 'completed' || iceState === 'complete' || (event && !event.candidate)) {
        ...
        // answerDesc  = getRemoteSDP(answerDesc.sdp)
		console.warn("answerDesc:",answerDesc.sdp)
